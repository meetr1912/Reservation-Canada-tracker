"""Unit tests for the network-free helpers in notify.py (email-only alerts)."""
...
VALID = {"email": "a@b.com", "parks": ["Fundy - Headquarters"],
...
def test_parse_ignores_legacy_phone_carrier_keys():
    # Older issues still carry phone/carrier; they must parse fine and be dropped.
    legacy = {**VALID, "phone": "5551234567", "carrier": "pcs.rogers.com"}
    c = notify.parse_alert_block(block(legacy))
    assert c == {"email": "a@b.com", "parks": ["Fundy - Headquarters"],
                 "start": "2026-07-04", "end": "2026-07-06"}
...
def test_matches_respects_range_park_and_today():
    c = {"email": "a@b.com", "parks": ["Fundy - Headquarters"],
         "start": "2026-07-04", "end": "2026-07-06"}
...
def test_matches_any_park_when_empty():
    c = {"email": "a@b.com", "parks": [], "start": "2026-07-04", "end": "2026-07-04"}
    m = notify.matches_for(REPORT, c, date(2026, 7, 1))
...
def test_matches_clamped_to_today():
    c = {"email": "a@b.com", "parks": [], "start": "2026-07-01", "end": "2026-07-31"}
...
def test_email_includes_per_opening_links():
