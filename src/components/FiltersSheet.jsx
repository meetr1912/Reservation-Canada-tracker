      ...
      >
        ...
        <div className="flex items-center gap-3 border-t border-gray-100 px-5 py-3 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
          ...
          <button
            type="button"
            onClick={onApply}
            className="h-11 flex-1 rounded-xl bg-emerald-600 text-sm font-semibold text-white transition-colors hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-500/40"
          >
            {resultCount === 1 ? 'Show 1 result' : `Show ${resultCount} results`}
          </button>
        </div>
      </SheetContent>
    </Dialog>
