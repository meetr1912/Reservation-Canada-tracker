...
class ParksCanadaTracker:
    ...
    def scan(self):
        ...
        # DEBUG: Target specific site mentioned by user
        inventory = [s for s in inventory if str(s['resourceId']) == "-2147480411"]
        print(f"DEBUG: Targeted test on {len(inventory)} sites: {inventory}")
        ...
        with sync_playwright() as p:
            ...
            for idx, site in enumerate(inventory):
                site_id = site['resourceId']
                print(f"  [{idx}/{len(inventory)}] Scanning {site_id} ({site['park']})...")
                ...
                for start_str, end_str in chunks:
                    ...
                    try:
                        # Use a timeout of 5s per request
                        response = page.request.get(url, timeout=5000)
                        if response.ok:
                            data = response.json()
                            print(f"    DEBUG: Data type: {type(data)}")
                            if isinstance(data, dict):
                                print(f"    DEBUG: First 2 keys: {list(data.keys())[:2]}")
                                for d_str, entry in data.items():
                                    if isinstance(entry, dict) and entry.get('availability') == 1:
                                        print(f"    DEBUG: Found {d_str} for -2147480411")
                                        try:
                                ...
                                if len(data) > 0:
                                    print(f"    DEBUG: First entry keys: {list(data[0].keys()) if isinstance(data[0], dict) else 'Not a dict'}")
                                    print(f"    DEBUG: First entry: {data[0]}")
                                
                                # Use index-based date if 'date' is missing
                                for i, entry in enumerate(data):
                                    if isinstance(entry, dict) and entry.get('availability') == 1:
                                        # Assume index corresponds to day offset from start_str
                                        base_date = datetime.strptime(start_str, "%Y-%m-%d")
                                        avail_date = base_date + timedelta(days=i)
                                        ...
                                        site_found += 1
                                        print(f"    DEBUG: Derived date {avail_date.strftime('%Y-%m-%d')} for index {i}")
                            else:
                                print(f"    DEBUG: Data is not a list/dict? {type(data)}")
                        else:
                            print(f"    DEBUG: API Request Failed with status {response.status}")
                    except Exception as e:
                        print(f"    DEBUG: Request Exception: {e}")
                
                print(f"    DEBUG: Finished all chunks for {site_id}. Total found: {site_found}")
                
                if site_found > 0:
                    print(f"    -> Found {site_found} openings for {site['siteName']}")
    ...
    def _save_report(self, raw_results):
        """Saves the availability report in JSON format."""
        
        # Load the full list of oTENTiks to include all of them in the report
        try:
            with open("otentiks.json", "r") as f:
                all_otentiks = json.load(f)
        except FileNotFoundError:
            print("otentiks.json not found. Please ensure the file exists.")
            return
        except json.JSONDecodeError:
            print("Error decoding otentiks.json.")
            return

        # Create a set of available site-dates for quick lookup
        available_dates = {
            (result['site_id'], result['date'].strftime('%Y-%m-%d'))
            for result in raw_results
        }

        # Generate the report for the next 6 months
        report = {}
        today = datetime.now()
        for i in range(180): # 6 months ~ 180 days
            ...
            daily_availability = []
            for otentik in all_otentiks:
                site_id = otentik.get('ResourceId')
                
                # The status is true if the site is in our set of available dates
                status = (str(site_id), date_str) in available_dates
            ...
            report[date_str] = daily_availability

        # Write the JSON report
        with open("availability_report.json", "w") as f:
            json.dump(report, f, indent=2)
        
        print("\nScan Complete! Report saved to availability_report.json")

        # Optional: Keep email functionality if needed, but format for email might need adjustment
        try:
            # Create a simple summary for the email
            summary = f"oTENTik Availability Report - {datetime.now().strftime('%Y-%m-%d')}\n"
            summary += f"Found {len(raw_results)} available slots in the next 6 months.\n"
            summary += "Full details are in the attached availability_report.json file."
